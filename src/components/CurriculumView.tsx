...
export const CurriculumView: React.FC<CurriculumViewProps> = ({ onBackToPortfolio }) => {
  ...
  return (
    <div id="curriculum-page" className="pt-40 sm:pt-32 pb-20 relative overflow-x-hidden w-full">
      {/* Background Accent Shapes wrapped in overflow-hidden to prevent horizontal scrolling */}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div className="relative pl-6 sm:pl-8 space-y-6 sm:space-y-8 before:absolute before:left-2.5 sm:before:left-3.5 before:top-3 before:bottom-3 before:w-0.5 before:bg-[#E9D5FF]">
            {RESUME_DATA.experiences.map((exp) => (
              <div key={exp.id} className="relative group/exp">
                {/* Timeline node with glowing micro-interaction */}
                <div className="absolute -left-[20px] sm:-left-[25px] top-2.5 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-white border-3 sm:border-4 border-[#7C3AED] shadow-sm group-hover/exp:scale-125 group-hover/exp:border-[#6D28D9] group-hover/exp:ring-4 group-hover/exp:ring-[#EDE9FE] transition-all duration-300" />
